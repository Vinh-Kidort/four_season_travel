...
function TourDetail() {
  ...
  if (!tour) return (
    <div className="text-center py-20 text-red-500 text-lg font-bold">
      {t('tour.notFoundDetail')}
    </div>
  );

  // Chuẩn hóa images
  const images = (tour.images || []).map(img =>
    typeof img === 'string' ? { url: img, caption: '', captionEn: '' } : img
  );
  ...
  const tourRegion        = isEng && tour.regionEn            ? tour.regionEn            : tour.region;
    ...
    <div className="bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(tour.isRejected || !tour.isApproved) && (
          <div className="mb-4 sm:mb-6 bg-red-50 border border-red-200 rounded-xl
            p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <span className="text-2xl flex-shrink-0">🚫</span>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-red-700 text-sm sm:text-base">
                {t('tourDetail.inactiveTitle')}
              </p>
              <p className="text-red-500 text-xs sm:text-sm mt-0.5">
                {t('tourDetail.inactiveDesc')}
              </p>
            </div>
            <Link to="/tours"
              className="flex-shrink-0 bg-red-500 text-white px-3 sm:px-4 py-2
                rounded-lg text-xs sm:text-sm font-bold hover:bg-red-600 transition">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3
            mb-4 sm:mb-8 pb-4 sm:pb-6 border-b border-gray-100">
            ...
            <span className={`px-2 sm:px-3 py-1 rounded-md text-xs font-bold
              ...
              {tour.status === 'active' ? t('tourDetail.onSale') : t('tourDetail.closed')}
            </span>

            <div className="flex items-center text-gray-600 text-xs sm:text-sm
              font-medium bg-gray-100 px-2 sm:px-3 py-1 sm:py-1.5 rounded-md">
              <span className="mr-1">⏱️</span> {tourDuration}
            </div>
            ...
            <div className={`flex items-center text-xs sm:text-sm font-medium
              ...
              {t('tourDetail.spotsLeft', { count: tour.availableSlots })}
            </div>

            {tourRegion && (
              <div className="hidden sm:flex items-center text-gray-600 text-sm
                font-medium bg-gray-100 px-3 py-1.5 rounded-md">
                <span className="mr-1.5">📍</span> {tourRegion}
              </div>
            )}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-bold text-gray-800 text-sm">{tour.averageRating || 0}</span>
              <span className="text-gray-400 text-xs">
                ({tour.reviewCount || 0} {t('tourDetail.reviews')})
              </span>
            </div>
        ...
        {/* Gallery */}
        <div className="relative rounded-2xl overflow-hidden mb-8 sm:mb-12 bg-gray-100
          h-52 sm:h-80 md:h-[500px]">
          {images.length === 0 ? (
            ...
            <div className="grid grid-cols-4 grid-rows-2 gap-2 h-full">
              <div className="col-span-2 row-span-2 relative cursor-pointer group"
                ...
                <img src={images[0].url} alt={displayName}
                  className="w-full h-full object-cover group-hover:brightness-90 transition duration-300" />
              </div>
              {images.slice(1, 5).map((img, idx) => (
                <div key={idx} className="hidden sm:block relative cursor-pointer group"
                  onClick={() => setLightbox(idx + 1)}>
                  <img src={img.url} alt={`tour-${idx}`}
                    className="w-full h-full object-cover group-hover:brightness-90 transition duration-300" />
          ...
          {images.length > 0 && (
            <button onClick={() => setLightbox(0)}
              className="absolute bottom-4 right-4 bg-white text-gray-900 px-4 py-2
                rounded-lg text-sm font-semibold border border-gray-900 hover:bg-gray-100
                transition shadow-sm z-10 flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path d="M5 3a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2H5zM5 11a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2H5zM11 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zM11 13a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
              </svg>
              {t('tourDetail.viewAllPhotos', { count: images.length })}
            </button>
          )}
        </div>
          ...
          <div className="lg:w-2/3">
                      ...
                      <button key={dep.id}
                        ...
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="font-bold text-gray-800">
                              ...
                              <span className="ml-2 text-sm font-normal text-gray-500">
                                ({dep.totalDays} {t('tourDetail.days')})
                              </span>
                            </p>
                            <p className={`text-sm mt-0.5 ${
                              ...
                              {isFull
                                ? t('tourDetail.full')
                                : t('tourDetail.available', { count: dep.availableSlots })}
                            </p>
                          </div>
                          <div className="text-right">
                            ...
                            <p className="text-xs text-gray-400">{t('tourDetail.perPerson')}</p>
                          </div>
                        </div>
                        {dep.note && (
                          <p className="text-xs text-orange-500 mt-2 flex items-center gap-1">
                            <span>⚠️</span>
                            {isEng && dep.noteEn ? dep.noteEn : dep.note}
                          </p>
                        )}
                      </button>
                    );
            ...
            {activeDepartures.length === 0 && (
              <div className="text-center py-6 bg-gray-50 rounded-xl border
                border-dashed border-gray-200 mb-8">
                {(tour.departures || []).length > 0 ? (
                  <>
                    <p className="text-gray-400 text-sm">🗓️ {t('tourDetail.depsEnded')}</p>
                    <p className="text-gray-400 text-xs mt-1">{t('tourDetail.checkBackLater')}</p>
                  </>
                ) : (
                  <>
                    <p className="text-gray-400 text-sm">🗓️ {t('tourDetail.noDeps')}</p>
                    <p className="text-gray-400 text-xs mt-1">{t('tourDetail.contactUs')}</p>
                  </>
                )}
              </div>
            )}
            ...
            {images.length > 0 && (
              <div className="py-8">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">
                  {t('tourDetail.experience')}
                </h2>
                {displayExperience && (
                  <div className="prose prose-blue max-w-none text-gray-600
                    leading-relaxed whitespace-pre-line mb-8 text-lg">
                    {displayExperience}
                  </div>
                )}
                  ...
                  {images.map((img, idx) => (
                    <div key={idx} className="group">
                      <div className="overflow-hidden rounded-xl bg-gray-100 shadow-sm
                        cursor-pointer" onClick={() => setLightbox(idx)}>
                        <img src={img.url}
                          alt={isEng && img.captionEn ? img.captionEn : (img.caption || `Photo ${idx + 1}`)}
                          className="w-full h-auto max-h-[500px] object-cover
                            group-hover:scale-[1.02] transition duration-500" />
                      </div>
                      {(img.caption || img.captionEn) && (
                        <p className="mt-3 text-sm text-gray-500 italic text-center">
                          ▲ {isEng && img.captionEn ? img.captionEn : img.caption}
                        </p>
                      )}
                    </div>
                  ))}
            ...
            <div className="py-10 border-t border-gray-200">
              <CommentSection itemId={tour.id} itemType="TOUR" />
            </div>
          </div>

          {/* Cột phải — booking card */}
          <div className="lg:w-1/3">
            <div className="sticky top-24 bg-white border border-gray-200
              ...
              {activeDepartures.length > 0 ? (
                <button
                  onClick={() => {
                    if (selectedDep) navigate(`/booking/${tour.id}?depId=${selectedDep.id}`);
                  }}
                  className={`w-full block text-center font-bold text-lg py-4
                    rounded-xl text-white transition duration-200 ${
                    selectedDep
                      ? 'bg-blue-600 hover:bg-blue-700 shadow-md cursor-pointer'
                      : 'bg-gray-300 cursor-not-allowed'
                  }`}>
                  {selectedDep ? t('tour.bookTour') : t('tourDetail.selectDateFirst')}
                </button>
              ) : (
                <div className="w-full text-center py-4 bg-amber-50 border
                  border-amber-200 rounded-xl">
                  <p className="text-amber-700 font-bold text-sm">
                    🗓️ {t('tourDetail.noSchedule')}
                  </p>
                  <p className="text-amber-500 text-xs mt-1">
                    {t('tourDetail.checkBackLater')}
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white
        ...
        shadow-[0_-10px_20px_rgba(0,0,0,0.05)] flex items-center justify-between">
        <div>
        ...
        {activeDepartures.length > 0 ? (
          <button
            ...
            className={`font-bold px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl
              text-white text-sm sm:text-base transition ${
              selectedDep
                ? 'bg-blue-600 active:bg-blue-800'
                : 'bg-gray-300 cursor-not-allowed'
            }`}>
            {selectedDep ? t('tourDetail.bookNow') : t('tourDetail.selectDate')}
          </button>
        ) : (
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-95 z-[100] flex flex-col
          items-center justify-center p-4" onClick={() => setLightbox(null)}>
          <button className="absolute top-4 right-5 text-white text-4xl
            hover:text-gray-300 transition leading-none z-10"
            onClick={() => setLightbox(null)}>×</button>
          <span className="absolute top-5 left-1/2 -translate-x-1/2 text-white text-sm
            bg-white bg-opacity-20 px-4 py-1 rounded-full backdrop-blur-sm">
            {lightbox + 1} / {images.length}
          </span>

          {images.length > 1 && (
            <button onClick={e => { e.stopPropagation(); setLightbox(i => (i - 1 + images.length) % images.length); }}
              className="absolute left-2 md:left-8 top-1/2 -translate-y-1/2 text-white
                text-4xl md:text-6xl hover:text-gray-300 transition p-4">‹</button>
          )}

          <div className="flex flex-col items-center gap-4 max-w-5xl w-full"
            onClick={e => e.stopPropagation()}>
            <img src={images[lightbox]?.url}
              alt={isEng && images[lightbox]?.captionEn
                ? images[lightbox].captionEn
                : (images[lightbox]?.caption || '')}
              className="max-h-[80vh] max-w-full object-contain rounded-md" />
            {(images[lightbox]?.caption || images[lightbox]?.captionEn) && (
              <p className="text-gray-300 text-sm text-center italic px-4 text-lg">
                ▲ {isEng && images[lightbox]?.captionEn
                    ? images[lightbox].captionEn
                    : images[lightbox]?.caption}
              </p>
            )}
          ...
          {images.length > 1 && (
            <button onClick={e => { e.stopPropagation(); setLightbox(i => (i + 1) % images.length); }}
              className="absolute right-2 md:right-8 top-1/2 -translate-y-1/2 text-white
                text-4xl md:text-6xl hover:text-gray-300 transition p-4">›</button>
          )}
        </div>
      )}
